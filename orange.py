...
def donut():
    ...
    degTheta = math.degrees(math.asin((branch_width/2)/r))
    turn(-degTheta)
    pen_down()
    turn(180)
    arc(r, 360 - (2*degTheta), "blue", 5)
...
def arc(radius, AngleTrav, col, colNum):
    global x,y,theta
    newx = x + radius*math.sin(math.radians(theta))
    newy = y + radius*math.cos(math.radians(theta))
    w.create_arc(int(newx + radius), int(newy + radius), int(newx - radius), int(newy - radius), start = theta + 90, extent = AngleTrav, style=ARC, width = 1, outline= col)

    arc = dxf.arc(dxf_scale*radius,(dxf_scale*newx , dxf_scale*newy), -(theta + 90+AngleTrav), -(theta + 90))
    arc['layer'] = 'LINES'
    arc['color'] = colNum
    drawing.add(arc)
...
"edited version from from Garcia file 'star' function to account for the branch width"
def star():
    global total_objects
    total_objects += 1
    pen_up()
...
"""
def vee():
    choices = (pentagon, donut, star, vee, vee)
    global x, y, theta
    (oldx,oldy,oldtheta) = (x,y,theta)
    pen_up()
    turn(90)
